...
if [[ "${libtorrent:-no}" == yes ]]; then
	...
	#
	"${build_d}/boost_1_${boost_v}_0/b2" -j"$(nproc)" address-model="$(getconf LONG_BIT)" "${lto}" optimization=speed cxxstd="${cxxstd}" variant=release dht=on encryption=on crypto=openssl i2p=on extensions=on threading=multi link=static boost-link=static install --prefix="${install_d}"
	#
...
if [[ "${python_b}" == yes ]]; then
	...
	#
	"${build_d}/boost_1_${boost_v}_0/b2" -j"$(nproc)" address-model="$(getconf LONG_BIT)" fpic=on "${lto}" optimization=speed cxxstd="${cxxstd}" variant=release dht=on encryption=on crypto=openssl i2p=on extensions=on threading=multi libtorrent-link=static boost-link=static install_module python-install-scope=user
	#
